...
get_value_command::get_value_command( session& s, const sheet_range& p )
    : base_command<i_command>::base_command( s )
    , m_position( p )
{}

void get_value_command::execute()
{
    ...
    m_value = r.value( m_session );
}

get_value_command::result_type get_value_command::value() const
{
    return m_value;
}

DEFINE_CLONEABLE( get_value_command );
