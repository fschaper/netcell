/*
 * \brief class Sheet representing worksheet
 *
 * \file Sheet.js
 *
...
Jedox.wss.grid.Sheet = function (cb, book, uid)
{
	...
	this._book = book; this.holder = book.holder; this._uid = uid; this._id = book._sheetCnt++; this._domId = book._domId.concat('S', this._id); this.dom = document.createElement('div');
	this._scrollReg = [ this._hScrollReg = {}, this._vScrollReg = {} ];
	this._els = {}; this._elZIdx = Jedox.wss.wsel.wselRegistry.startZIdx - 1;
	this._cmnts = {}; this._cmntMoveCCoord = [ -1, -1 ];


	// aliases
	this._viewMode = Jedox.wss.app.appMode; this._app = Jedox.wss.app; this._jwgrid = Jedox.wss.grid; this._jwstyle = Jedox.wss.style; this._conn = Jedox.backend;
	this._cellSizeIncr = this._jwgrid.defCellSizeIncr;
	this._setText = Jedox.util.setText; this._n2l = Jedox.wss.app.numberToLetter; this._gmode_edit = book._gmode_edit;
	...
	// fetch sheet data
	this._conn.ccmdSetBuff('[["gdcr",2],["gcr",0,1,'.concat(this._defMaxCoords[0] - 1, '],["gcr",1,1,', this._defMaxCoords[1] - 1, '],["gurn"]]'));
	this._conn.ccmdFlush([ this, this._init_post, cb ]);


	// environment
	this._env = new Jedox.wss.app.Environment();
  ...
  , _init_post: function (res, cb)
	{
		...
		// set column/row sizes and used range
		...
		this._paneConf = [ { range: [ 1, 1, this._defMaxCoords[0], this._defMaxCoords[1] ], pos: { c: 1, r: 1 }, hscroll: true, vscroll: true } ];
		...
		if (this._gmode_edit)
		{
			...
			{
				this._rowHdrsOC0.style.height = ''.concat(p0Conf.coords.h, 'px');
				...
				this._rowHdrsOC1.scrollTop = this._paneConf[this._crosshairs[0] ? 2 : 1].pos.t;
			}
			...
			// first
			this._colHdrs0 = []; this._rowHdrs0 = [];

			this._hdrArrs0 = [ this._colHdrs0, this._rowHdrs0 ];
			this._activeHdrs_o0 = [ {}, {} ]; this._activeHdrs_a0 = [ [], [] ];

			for (var ch = this._colHdrs0, hdrs = this._colHdrsIC0.getElementsByTagName('div'), i = this._numCols0 - 1; i >= 0; --i)
				ch[i] = hdrs[i];

			for (var rh = this._rowHdrs0, hdrs = this._rowHdrsIC0.getElementsByTagName('div'), i = this._numRows0 - 1; i >= 0; --i)
				rh[i] = hdrs[i];

			this._colHdrsIC0.onmousemove = function (ev) { Jedox.wss.mouse.checkForResize(ev, that); };
			this._colHdrsIC0.onmousedown= function (ev) { that._defaultSelection.selectOrResize(ev); document.body.onselectstart = Jedox.util.ignoreEvent; };
			this._colHdrsIC0.ondblclick = Jedox.wss.mouse.autoSizeCell;

			this._rowHdrsIC0.onmousemove = function (ev) { Jedox.wss.mouse.checkForResize(ev, that); };
			this._rowHdrsIC0.onmousedown = function (ev) { that._defaultSelection.selectOrResize(ev); document.body.onselectstart = Jedox.util.ignoreEvent; };
			this._rowHdrsIC0.ondblclick = Jedox.wss.mouse.autoSizeCell;


			// second
			this._hdrArrs1 = [];
			this._activeHdrs_o1 = [ {}, {} ]; this._activeHdrs_a1 = [ [], [] ];
			...
			if (this._crosshairs[0])
			{
				...
				for (var ch = this._colHdrs1, hdrs = this._colHdrsIC1.getElementsByTagName('div'), i = this._numCols1 - 1; i >= 0; --i)
					ch[i] = hdrs[i];

				this._colHdrsIC1.onmousemove = function (ev) { Jedox.wss.mouse.checkForResize(ev, that); };
				this._colHdrsIC1.onmousedown= function (ev) { that._defaultSelection.selectOrResize(ev); document.body.onselectstart = Jedox.util.ignoreEvent; };
				this._colHdrsIC1.ondblclick = Jedox.wss.mouse.autoSizeCell;
			}
			...
			if (this._crosshairs[1])
			{
				...
				for (var rh = this._rowHdrs1, hdrs = this._rowHdrsIC1.getElementsByTagName('div'), i = this._numRows1 - 1; i >= 0; --i)
					rh[i] = hdrs[i];

				this._rowHdrsIC1.onmousemove = function (ev) { Jedox.wss.mouse.checkForResize(ev, that); };
				this._rowHdrsIC1.onmousedown = function (ev) { that._defaultSelection.selectOrResize(ev); document.body.onselectstart = Jedox.util.ignoreEvent; };
				this._rowHdrsIC1.ondblclick = Jedox.wss.mouse.autoSizeCell;
			}


			document.getElementById(this._domId + '_gridSelectAll').onmousedown = function (ev) { that._defaultSelection.markAllCells(ev); };

			document.body.onmouseup = function () { document.body.onselectstart = Jedox.util.unignoreEvent; };
		}
		else
		{
			this._activeHdrs_o0 = [ [], [] ]; this._activeHdrs_a0 = [ [], [] ];
		...
		{
			this['_paneOC'.concat(i)] = this._paneOCs[i] = paneOC = document.getElementById(this._domId.concat('P', i, '_OC'));
			this._panes[i] = pane = new Jedox.wss.grid.Pane(this, i, paneOC, batch);
		...
		{
			batch.setOnSuccess([ this, this._initFloatEl, cb ]);
			batch.send();

			this._loadRqst = batch;
		}
	...
	, _initFloatEl: function (cb)
	{
		...
		if (this._gmode_edit)
		{
			var col = this._paneConf[0].pos.c,
					row = this._paneConf[0].pos.r;

			this._env.shared.defaultSelection = this._defaultSelection = new Jedox.wss.grid.DefaultSelection(new Jedox.gen.Point(col, row), this);
			this._env.shared.formulaSelection = this._formulaSelection = new Jedox.wss.grid.FormulaSelection(new Jedox.gen.Point(col, row), this);
			this._env.shared.copySelection = this._copySelection = new Jedox.wss.grid.CopySelection(new Jedox.gen.Point(col, row), this);
			this._defaultSelection.selectFirstVisCell();
			this._defaultSelection.getCursorField().refresh();

			this._env.shared.autoScroll = this._autoScroll = new Jedox.wss.grid.AutoScroll(this);
		}
		else
			this._cursorField = new Jedox.wss.grid.CursorField(this._domId, this);
		...
		Jedox.wss.cmnt.loadAll();
		Jedox.wss.hb.loadAll(this);

		this._conn.ccmdFlush(cb);
	}
	...
	, _genContentEl: function ()
	{
		if (this._gmode_edit)
		{
			var hdrsWidth = this._HDRS_WIDTH,
					hdrsHeight = this._HDRS_HEIGHT,
			...
			if (this._crosshairs[0])
			{
				...
				{
					for (var w, xtra_c = 2, pos = this._paneConf[0].pos, c = pos.c, l = pos.l, l_max = l + this._crosshairs[0]; l <= l_max || xtra_c--; ++c, l += w)
						html.push('<div class="gridColHdr" style="left:', l, 'px;', ((w = this._colWidths.getElemAt(c)) ? 'width:'.concat(w - decr, 'px;') : 'display:none;'), '">', this._n2l[c], '</div>');
				...
				this._rngCols0 = [ this._paneConf[0].range[0], this._paneConf[0].range[2] ];
			...
			for (var w, xtra_c = 2, pos = this._paneConf[colPaneId].pos, c = pos.c, l = pos.l, l_max = l + colsSize; l <= l_max || xtra_c--; ++c, l += w)
				html.push('<div class="gridColHdr" style="left:', l, 'px;', ((w = this._colWidths.getElemAt(c)) ? 'width:'.concat(w - decr, 'px;') : 'display:none;'), '">', this._n2l[c], '</div>');

			html.push('</div></div>');

			this['_numCols'.concat(colPaneId)] = c - pos.c;
			this['_rngCols'.concat(colPaneId)] = [ this._paneConf[colPaneId].range[0], this._paneConf[colPaneId].range[2] ];
			...
			if (this._crosshairs[1])
			{
				...
				if (this._crosshairs[2])
				{
					for (var h, xtra_r = 2, pos = this._paneConf[0].pos, r = pos.r, t = pos.t, t_max = t + this._crosshairs[1]; t <= t_max || xtra_r--; ++r, t += h)
						html.push('<div class="gridRowHdr" style="top:', t, 'px;', ((h = this._rowHeights.getElemAt(r)) ? 'height:'.concat(h - decr, 'px;') : 'display:none;'), '">', r, '</div>');
				...
				this._rngRows0 = [ this._paneConf[0].range[1], this._paneConf[0].range[3] ];
			...
			for (var h, xtra_r = 2, pos = this._paneConf[rowPaneId + colPaneId].pos, r = pos.r, t = pos.t, t_max = t + rowsSize; t <= t_max || xtra_r--; ++r, t += h)
				html.push('<div class="gridRowHdr" style="top:', t, 'px;', ((h = this._rowHeights.getElemAt(r)) ? 'height:'.concat(h - decr, 'px;') : 'display:none;'), '">', r, '</div>');

			html.push('</div></div>');

			this['_numRows'.concat(rowPaneId)] = r - pos.r;
			this['_rngRows'.concat(rowPaneId)] = [ this._paneConf[rowPaneId + colPaneId].range[1], this._paneConf[rowPaneId + colPaneId].range[3] ];
		}
		else
		{
			var hdrsWidth = 0,
			...
			if (this._crosshairs[0])
			{
				if (this._crosshairs[2])
				{
					for (var xtra_c = 2, colsOffset = this._crosshairs[0], pos = this._paneConf[0].pos, c = pos.c, l = 0; l <= colsOffset || xtra_c--; l += this._colWidths.getElemAt(c++)) {}

					this._numCols0 = c - pos.c;
				}
				else
				{
					for (var cols = this._crosshairs[0], colsOffset = 0, c = this._paneConf[0].pos.c, i = 0; i < cols; ++i, colsOffset += this._colWidths.getElemAt(c++)) {}
			...
			for (var xtra_c = 2, pos = this._paneConf[colPaneId].pos, c = pos.c, l = 0; l <= colsSize || xtra_c--; l += this._colWidths.getElemAt(c++)) {}
			...
			if (this._crosshairs[1])
			{
				if (this._crosshairs[2])
				{
					for (var xtra_r = 2, rowsOffset = this._crosshairs[1], pos = this._paneConf[0].pos, r = pos.r, t = 0; t <= rowsOffset || xtra_r--; t += this._rowHeights.getElemAt(r++)) {}

					this._numRows0 = r - pos.r;
				}
				else
				{
					for (var rows = this._crosshairs[1], rowsOffset = 0, r = this._paneConf[0].pos.r, i = 0; i < rows; ++i, rowsOffset += this._rowHeights.getElemAt(r++)) {}
			...
			for (var xtra_r = 2, pos = this._paneConf[rowPaneId + colPaneId].pos, r = pos.r, t = 0; t <= rowsSize || xtra_r--; t += this._rowHeights.getElemAt(r++)) {}
		...
		{
			this._paneConf[0].coords = { l: hdrsWidth, t: hdrsHeight, w: colsOffset, h: rowsOffset, x: 0, y: 0 };
			this._paneConf[1].coords = { l: colsOffset + hdrsWidth, t: hdrsHeight, w: colsSize, h: rowsOffset, x: 1, y: 0 };
			this._paneConf[2].coords = { l: hdrsWidth, t: rowsOffset + hdrsHeight, w: colsOffset, h: rowsSize, x: 0, y: 1 };
			this._paneConf[3].coords = { l: colsOffset + hdrsWidth, t: rowsOffset + hdrsHeight, w: colsSize, h: rowsSize, x: 1, y: 1 };
		...
		else if (this._crosshairs[0])
		{
			this._paneConf[0].coords = { l: hdrsWidth, t: hdrsHeight, w: colsOffset, h: rowsSize, x: 0, y: 0 };
			this._paneConf[1].coords = { l: colsOffset + hdrsWidth, t: hdrsHeight, w: colsSize, h: rowsSize, x: 1, y: 0 };
		...
		else if (this._crosshairs[1])
		{
			this._paneConf[0].coords = { l: hdrsWidth, t: hdrsHeight, w: colsSize, h: rowsOffset, x: 0, y: 0 };
			this._paneConf[1].coords = { l: hdrsWidth, t: rowsOffset + hdrsHeight, w: colsSize, h: rowsSize, x: 0, y: 1 };
		...
		{
			this._paneConf[0].coords = { l: hdrsWidth, t: hdrsHeight, w: colsSize, h: rowsSize, x: 0, y: 0 };
		...
		html.push('<div class="HeaderMarker" id="', this._domId, '_startMarker"></div><div class="HeaderMarker" id="', this._domId, '_stopMarker"></div></div>');
	...
	, _regPane: function (reg, coord, pane)
	{
		if (!reg[coord])
			reg[coord] = [ pane ];
		else if (reg[coord].indexOf(pane) == -1)
			reg[coord].push(pane);
	}
	...
	{
		var book = this._book,
				sOCWidth = book._sheetsOCWidth,
				sheetSel = book._sheetSelector,
				hs2reg = {}, vs2reg = {},
				widths = [], heights = [],
				maxw0, hs, vs, reg;
		...
		if (this._hScrollReg[0])
			hs2reg = { 0: 0, 1: this._hScrollReg[1] ? 1 : false };
		else if (this._hScrollReg[1])
			hs2reg = { 0: 1, 1: false };
		else
			hs2reg = { 0: false, 1: false };
		...
		if (this._vScrollReg[0])
			vs2reg = { 0: 0, 1: this._vScrollReg[1] ? 1 : false };
		else if (this._vScrollReg[1])
			vs2reg = { 0: 1, 1: false };
		else
			vs2reg = { 0: false, 1: false };

		heights[1] = vs2reg[1] ? this._vScrollReg[1][0]._ocHeight : 0;
		heights[0] = vs2reg[0] !== false ? book._sheetsOCHeight - heights[1] : 0;
		...
		for (var coord in hs2reg)
		{
			...
			{
				book._hScrolls[coord] = new Jedox.wss.grid.HorizScrollbar(book, book._hsOCs[coord], this._hScrollReg[reg], widths[coord]);
				...
				new Ext.Resizable(book._hScrolls[0]._domId, {
				  transparent: true
				, handles: 'w'
				, listeners: { resize: function (obj, w)
					{
						if (w > book._hsMaxW)
							w = book._hsMaxW;
		...
		for (var coord in vs2reg)
		{
			...
			if (vs = book._vScrolls[coord])
				...
				book._vScrolls[coord] = new Jedox.wss.grid.VertScrollbar(book, book._vsOCs[coord], this._vScrollReg[reg], heights[coord]);
		}
	}
	...
	{
		if (col > this._farthestUsedCell[0])
			this._farthestUsedCell[0] = col;

		if (row > this._farthestUsedCell[1])
			this._farthestUsedCell[1] = row;
	...
	{
		if (range[2] > this._farthestUsedCell[0])
			this._farthestUsedCell[0] = range[2];

		if (range[3] > this._farthestUsedCell[1])
			this._farthestUsedCell[1] = range[3];
	...
	, setRangeLock: function (range, lock)
	{
		...
		this._conn.ccmd(true, [ 'lock', range, lock ]);
	...
	{
		var crd = this._colRowDims[type],
				csi = this._cellSizeIncr[type],
				clss = this._hdrClss[type].concat(' ', cls),
				sprop = type ? 'height' : 'width';

		for (var pos in this._hdrsICs1[type] ? { 0: true, 1: true } : { 0: true })
		{
			var hdrsIC = this['_hdrsICs'.concat(pos)][type],
					hdrrng = this['_rngColsRows'.concat(pos)][type],
					ahdrs_a = this['_activeHdrs_a'.concat(pos)],
					ahdrs_o = this['_activeHdrs_o'.concat(pos)],
					hdrs = ahdrs_a[type];
			...
			if (notfnd)
			{
				actHdr = this['_hdrArrs'.concat(pos)][type][idx - this._panes[this._panesByXY[type][pos]]._currTopCell[type]];
	...
	, activateHdrRng: function (type, rng, cls)
	{
		...
		for (var pos in this._hdrsICs1[type] ? { 0: true, 1: true } : { 0: true })
		{
			...
			var hdrArrs = this['_hdrArrs'.concat(pos)][type],
					pane = this._panes[this._panesByXY[type][pos]],
					tblsize = pane._tableSize[type],
					ctct = pane._currTopCell[type],
					actHdr;
	...
	, locateHdr: function (type, obj, pane)
	{
		...
		if (!pane)
			var span = this._hdrsICs1[type] ? { 0: true, 1: true } : { 0: true };
		else
		{
			var span = {};
		...
		for (var pos in span)
		{
			var ahdrs_a = this['_activeHdrs_a'.concat(pos)][type],
					ahdrs_o = this['_activeHdrs_o'.concat(pos)][type],
					pane = this._panes[this._panesByXY[type][pos]],
					hdrs = this['_hdrArrs'.concat(pos)][type];

			for (var i = pane._tableSize[type] - 1, ctci = pane._currTopCell[type] + i; i >= 0; --i, --ctci)
				if (hdrs[i] == obj || (ctci in ahdrs_o && ahdrs_a[ahdrs_o[ctci]][1] == obj))
					return [ ctci, pane ];
		}
	...
	, getHdrByCoord: function (type, coord, pane)
	{
		...
		return (type ? pane._rowHdrs : pane._colHdrs)[coord - pane._currTopCell[type]];
	}

	, insertCol: function (col, num)
	{
		this._conn.ccmd(true, [ 'ic', [ col, num ] ], true, Jedox.backend.Q_ALL, Jedox.backend.D_COLS);
	}

	, insertRow: function (row, num)
	{
		this._conn.ccmd(true, [ 'ir', [ row, num ] ], true, Jedox.backend.Q_ALL, Jedox.backend.D_ROWS);
	}

	, deleteCol: function (col, num)
	{
		this._conn.ccmd(true, [ 'dc', [ col, num ] ], true, Jedox.backend.Q_ALL, Jedox.backend.D_COLS);
	}

	, deleteRow: function (row, num)
	{
		this._conn.ccmd(true, [ 'dr', [ row, num ] ], true, Jedox.backend.Q_ALL, Jedox.backend.D_ROWS);
	}
	...
	, setColRowSize: function (type, chngsets)
	{
		...
		var span = this._crosshairs[type] ? { 0: true, 1: true } : { 0: true },
				csi = this._cellSizeIncr[type], crs = this._colRowDims[type], asSize = Jedox.util.asSize, def = crs.getDef(),
				sizes, range, size, sizepx, len, oldvals, i, pos,
				ahdrs_o, ahdrs_a, oldsize, st, tr_s, newpos, diff;
		...
		for (i = chngsets.length - 1; i >= 0; --i)
		{
			...
			for (pos in span)
			{
				var tbl_size = [ this['_numCols'.concat(pos)], this['_numRows'.concat(pos)] ],
						tbl_beg = this._panes[this._panesByXY[type][pos]]._currTopCell[type],
						tbl_end = tbl_beg + tbl_size[type] - 1;

				if (range[0] > tbl_end || range[1] < tbl_beg)
					continue;

				for (var min = Math.max(range[0], tbl_beg), c = Math.min(range[1], tbl_end); c >= min; --c)
					if (!(c in oldvals))
			...
			var range_diff = crs.getSumUpTo(range[1] + 1),
					pane_diff = 0;
			...
			for (pos in span)
				if (pos in move_ini)
					move_ini[pos] = crs.getSumUpTo(tbl_begs[pos]) - move_ini[pos];

			for (var pi = 0; pi < this._numPanes; ++pi)
			{
				var pane = this._panes[pi],
						pos = type ? pane._conf.coords.y : pane._conf.coords.x,
						th = pane._numRows, tw = pane._numCols, tm = pane._tableMat, ch = pane._colHdrs, cs = pane._colSeps, rh = pane._rowHdrs, fr = pane._firstRowCells, tr = pane._tableRows, ctc = pane._currTopCell,
						tbl_size = [ tw, th ],
						tbl_beg = ctc[type], tbl_end = tbl_beg + tbl_size[type] - 1,
						move_sum = move_ini[pos] | 0;

				if (type)
				{
					if (rh)
						ahdrs_o = this['_activeHdrs_o'.concat(pos)][type], ahdrs_a = this['_activeHdrs_a'.concat(pos)][type];
					else
				...
				for (var curr = range[0] > tbl_beg ? range[0] - tbl_beg : 0, max = range[1] < tbl_end ? range[1] - tbl_beg : tbl_end - tbl_beg, curr_abs = tbl_beg + curr, c = curr_abs - range[0]; curr <= max; ++curr, ++curr_abs)
				{
					if (len > 1)
					{
						if ((size = sizes[c++]) == -1)
					...
					if (type == this._SCROLL_HORIZ)
					{
						...
						else
						{
							...
							for (var mi, mrow, mcol, cell, cspx, row = ctc[1], j = 0; j < th; ++j, ++row)
							{
								...
								{
									if (mi[0] === true)
										cspx = asSize(crs.getSumUpTo(curr_abs + mi[2], curr_abs) - csi);
									else
									{
										cspx = st.borderLeftWidth != '2px' ? sizepx : asSize(size - csi - 1);

										if ((mrow = mi[1] + 1) == row && (mcol = mi[2] + 1) < range[0] && (cell = pane.getCellByCoords(mcol, mrow)) !== undefined)
											cell.style.width = asSize(parseInt(cell.style.width) + diff);
									}
								}
								else
									cspx = st.borderLeftWidth != '2px' ? sizepx : asSize(size - csi - 1);
					...
					else
					{
						...
						else
						{
							...
							for (var mi, mcol, mrow, cell, col = ctc[0], j = 0; j < tw; ++j, ++col)
							{
								...
								{
									if (mi[0] === true)
										st.height = asSize(crs.getSumUpTo(curr_abs + mi[1], curr_abs) - csi);
									else
									{
										if ((mcol = mi[2] + 1) == col && (mrow = mi[1] + 1) < range[0] && (cell = pane.getCellByCoords(mcol, mrow)) !== undefined)
											cell.style.height = asSize(parseInt(cell.style.height) + diff);

										if (st.borderTopWidth == '2px')
											st.height = asSize(size - csi - 1);
									}
								}
								else if (st.borderTopWidth == '2px')
									st.height = asSize(size - csi - 1);
							}
						}
				...
				if (this._crosshairs[type] && !this._crosshairs[2] && pos == 0 && move_sum)
					pane_diff = move_sum;
				...
				if (pane_diff)
				{
					if (type)
					{
						if (pos == 0)
						{
							...
							pane._oc.style.height = ''.concat(pane._ocHeight, 'px');

							if (pane._rowHdrsOC)
								pane._rowHdrsOC.style.height = ''.concat(pane._ocHeight, 'px');
						}
						else
						{
							pane._ocHeight = pane._conf.coords.h -= pane_diff;
							...
							pane._oc.style.top = ''.concat(pane._conf.coords.t, 'px');
							pane._oc.style.height = ''.concat(pane._ocHeight, 'px');

							if (pane._rowHdrsOC)
							{
								pane._rowHdrsOC.style.top = ''.concat(pane._conf.coords.t, 'px');
								pane._rowHdrsOC.style.height = ''.concat(pane._ocHeight, 'px');
							}
						}
					}
					else
					{
						if (pos == 0)
						{
							...
							pane._oc.style.width = ''.concat(pane._ocWidth, 'px');

							if (pane._colHdrsOC)
								pane._colHdrsOC.style.width = ''.concat(pane._ocWidth, 'px');
						}
						else
						{
							pane._ocWidth = pane._conf.coords.w -= pane_diff;
							...
							pane._oc.style.left = ''.concat(pane._conf.coords.l, 'px');
							pane._oc.style.width = ''.concat(pane._ocWidth, 'px');

							if (pane._colHdrsOC)
							{
								pane._colHdrsOC.style.left = ''.concat(pane._conf.coords.l, 'px');
								pane._colHdrsOC.style.width = ''.concat(pane._ocWidth, 'px');
							}
						}
					}
				...
				if (ahdrs_a)
					for (var ahdr, ahdr_s, curr = -1; (ahdr = ahdrs_a[++curr]) != undefined; )
					{
						...
						if (type == this._SCROLL_HORIZ)
							(ahdr_s = ahdr[1].style).left = ''.concat(parseInt(ahdr_s.left) + range_diff, 'px');
						else
							(ahdr_s = ahdr[1].style).top = ''.concat(parseInt(ahdr_s.top) + range_diff, 'px');
					}
				...
				for (var tr_s, newpos, max = tbl_size[type]; curr < max; ++curr)
					if (type == this._SCROLL_HORIZ)
					{
						newpos = ''.concat(parseInt(fr[curr].style.left) + range_diff, 'px');
					...
					{
						newpos = ''.concat(parseInt((tr_s = tr[curr].style).top) + range_diff, 'px');
		...
		// reposition & update virtual size
		for (var pane, i = this._numPanes - 1; i >= 0; --i)
		{
			pane = this._panes[i];
		...
		// re-draw selections
		if (this._gmode_edit)
			this._defaultSelection.draw(), this._formulaSelection.draw(), this._copySelection.draw(), Jedox.wss.hb.setAllNormal(null, true);
		else
		...
		for (var i = this._numPanes - 1; i >= 0; --i)
		{
			...
			var cr_size, pos = pane._lastDestCell[type] + 1, sum = crs.getElemAt(pos),
					oc_size = type == this._SCROLL_HORIZ ? pane._ocWidth : pane._ocHeight;

			for (var max = oc_size; sum < max; sum += crs.getElemAt(++pos)) {}

			pane._cppi[type] = pos - pane._lastDestCell[type] - 1;
			pane._cpp[type] = pane._cppi[type] + ((cr_size = crs.getElemAt(pos)) - sum + oc_size) / cr_size;
		}
	...
	, resizeColRow: function (type, ranges, newsize)
	{
		...
		if (ranges.length == 1 && ranges[0][0] == 1 && ranges[0][1] == this._defMaxCoords[type])
			...
			this._conn.ccmd(true, [ 'scr', type ].concat(chngsets));
	}

	, autofitColRow: function (type, ranges)
	{
		this._conn.ccmd(true, [ 'afit', type ].concat(ranges), true, Jedox.backend.Q_NONE, type + 1);
	}
	...
	, newDims: function (type, dims_new)
	{
		var svec_old = this._colRowDims[type],
				...
				idx, idx_old = idx_max, idx_new = idx_max,
				val, val_old = 0, val_new = 0,

				next_new = true, next_old = true,
				cond = 2,
		...
		for (var i_old = 0, i_new = 0, end_old = dims_old.length, end_new = dims_new.length; cond; )
		{
			...
			if (!set)
				set = [ idx, idx, val ];
			else if (idx - 1 == set[1] && (val == set[2] || set[1] - set[0] == set.length - 3))
			{
				++set[1];
	...
	, getPanesByCoords: function (col, row)
	{
		...
		for (var rng, i = this._numPanes - 1; i >= 0; --i)
		{
			...
			if (col >= rng[0] && col <= rng[2] && row >= rng[1] && row <= rng[3])
