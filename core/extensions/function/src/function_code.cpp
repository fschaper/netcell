...
struct calculate_code {
    static variant transform( const variant& v ) {
        try { // text representations of numbers are evaluated.
            const std::string& text = v.string(); // the text for which you want the code of the first character.
            if ( !text.empty() ) {
                return variant( static_cast<double>( ( int )text[0] ) );
            }
...
interpreter_variant function_code( function_parameter& parameters )
{
    ...
    return function_helper::create_array_or_return_single_value<calculate_code>( parameters[0] );
    // Returns a numeric code for the first character in a text string.
    // The returned code corresponds to the character set used by your computer.
}
