 ...
 *
 * \version
 * SVN: $Id: Border.js 4789 2011-03-30 09:45:56Z andrejv $
 *
 */

Jedox.wss.dlg.format.border = function(callback, _pre, toDisable, isFromOther, tabOffset, displayClear) {
	// format border
	...
	var _mxVert = ' background: url(/ui/wss/res/img/dialog/format_cells/vertical_null.png) 0 0;';
	var _mxHorz = ' background: url(/ui/wss/res/img/dialog/format_cells/horizontal_null.png) 0 0;';

	var borderColorStyle = '000000';
	var borderStyleList = [
	                       [ 'none', '<div style="line-height: 10px;">'.concat('None'.localize(),'</div>') ],
	                       [
	                        'solid',
	...
	var borderField = [
	                   [
	                    false,
	                    'top',
	                    '<div style="position:absolute; top:0px; left:0px; width: 146px; height: 20px;'.concat(_mixed.top? _mxHorz : '', '"></div>')],
	                    [
	                     false,
	                     'left',
	                     '<div style="position:absolute; top:0px; left:0px; width: 20px; height: 94px;'.concat(_mixed.left? _mxVert : '', '"></div>')],
	                     [
	                      false,
	                      'right',
	                      '<div style="position:absolute; top:0px; left:126px; width: 20px; height: 94px;'.concat(_mixed.right? _mxVert : '', '"></div>') ],
	                      [
	                       false,
	                       'bottom',
	                       '<div style="position:absolute; top:74px; left:0px; width: 146px; height: 20px;'.concat(_mixed.bottom? _mxHorz : '', '"></div>') ] ];

	// this is for 2x2 cell layout
	var borderField2x2 = [
	                      [
	                       false,
	                       'top',
	                        ...
	                        [
	                         ...
	                         [
	                          ...
	                          [
	                           false,
	                           'middle-hor',
	                           '<div style="position:absolute; top:38px; left:0px; width: 146px; height: 20px;'.concat(_mixed.ins_horiz? _mxHorz : '', '"></div>')],
	                           [
	                            false,
	                            'middle-vert',
	                            '<div style="position:absolute; top:0px; left:64px; width: 20px; height: 94px;'.concat(_mixed.ins_vert? _mxVert : '', '"></div>')] ];

	// this is column layout
	var borderFieldCol = [
	                      [
	                       false,
	                       'top',
	...
	} else if ((xCoordEC - xCoordSC == 0) && (yCoordEC - yCoordSC > 0)) {
	...
	//all layouts - single cell, 2x2, col, row will be loaded with this store
	...
	var topBorderHandler = function() {
		if (!topBorderProp || borderColorStyle!=borders.top.color || cellBorderStyle!=borders.top.style || borders.top.dim!=borderThickness) {
		...
		_mixed.top = false;
		borderDataView.refresh(); // refresh all
	};

	var midHorBorderHandler = function() {
		if (!midHorBorderProp || borderColorStyle!=borders.midHor.color || cellBorderStyle!=borders.midHor.style || borders.midHor.dim!=borderThickness) {
			tmpRec = new BorderTrueProp(
					{
						trigger : true,
						desc : 'mid-hor',
	...
	if (!midVerBorderProp || borderColorStyle!=borders.midVer.color || cellBorderStyle!=borders.midVer.style || borders.midVer.dim!=borderThickness) {
		tmpRec = new BorderTrueProp(
				{
					trigger : true,
					desc : 'mid-ver',
	...
	var bottomBorderHandler = function() {
		if (!bottomBorderProp || borderColorStyle!=borders.bottom.color || cellBorderStyle!=borders.bottom.style || borders.bottom.dim!=borderThickness) {
			tmpRec = new BorderTrueProp(
					{
						trigger : true,
						desc : 'top',
		...
		//	first remove present formatting
	...
	var leftBorderHandler = function() {
		...
		//	then add new one
		if (!leftBorderProp || borderColorStyle!=borders.left.color || cellBorderStyle!=borders.left.style || borders.left.dim!=borderThickness) {
			tmpRec = new BorderTrueProp(
					{
						trigger : true,
						desc : 'top',
	...
	var rightBorderHandler = function() {
		...
		//	then add new one
		if (!rightBorderProp || borderColorStyle!=borders.right.color || cellBorderStyle!=borders.right.style || borders.right.dim!=borderThickness) {
			tmpRec = new BorderTrueProp(
					{
						trigger : true,
						desc : 'top',
	...
	var allBorderHandler = function(dView, index, node, ev) {
		if (borderBgStyle == 'border-choose-field-2x2') {
			if (index == 0) {
				...
				midVerBorderHandler();
		} else if (borderBgStyle == 'border-choose-field-col') {
			if (index == 0) {
				topBorderHandler();
			} else if (index == 1) {
				leftBorderHandler();
				...
				midHorBorderHandler();
		} else if (borderBgStyle == 'border-choose-field-row') {
			if (index == 0) {
				topBorderHandler();
			} else if (index == 1) {
				leftBorderHandler();
				...
				midVerBorderHandler();
		} else if (borderBgStyle == 'border-choose-field') {
			if (index == 0) {
				topBorderHandler();
			} else if (index == 1) {
				leftBorderHandler();
	...
	var setBordersColor = function(colorP, colorStr) {
		...
		tmpRec0 = new borderStyleTmpList(
				{
					style : 'none',
					description : '<div style="line-height: 10px;">'.concat('None'.localize(),'</div>')
				});
	...
	var borderTab = new Ext.Panel(
			...
			afterlayout : function(th, l) {
				...
				if (!rendered){
					rendered = true;
					setTimeout( function() {
						// here preselection of borders

						for ( var y in borders) {
							if ((borders[y].style != '') && (borders[y].style != 'none') && (borders[y].style != undefined)) {
								borderThickness = borders[y].dim || '';
								cellBorderStyle = borders[y].style || '';
								borderColorStyle = borders[y].color || '';
								borderColorStyle = borderColorStyle.replace(/#/i, "");
								switch (y) {
								case 'top':
									topBorderHandler();
									break;
						...
						_setupColorButton2(borderColorButton, state.colorBrd, false);
						//colorSelectView.select(1);
						//colorSelectView.fireEvent('click', colorSelectView, 1);
					}, 300);
				}
			}
			},
	...
	function genBordersConf() {
		// Code for BordersConf generation comes here.

		// TODO: change to support undefined - mixed border
		// Sample:
		var brdConf = {}
		...
		if (_mixed.top) delete(brdConf.top);

		for (var b in _mixed){
			if (_mixed[b]) delete brdConf[b];
		}
