...
public:
    ...
    operator variant() {
        ...
        // If standard deviation equals zero, RSQ returns the #DIV/0! error value.
        if ( sx <= 0.0 ) {
            return  variant( variant::error_div_null );
        }

        double result = sumy2 - sumy * sumy / m_count;
        result -= pow( sumxy - sumx * sumy / m_count, 2.0 ) / sx;
        return  variant( sqrt( result / ( m_count - 2.0 ) ) ) ;
