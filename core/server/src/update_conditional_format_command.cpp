...
update_conditional_format_command::update_conditional_format_command( session& s, const conditional_format_lookup_id_type id, const set_conditional_format& cf_format, const shared_worksheet_type& ws /* = shared_worksheet_type */ )
...
void update_conditional_format_command::execute()
{
    shared_worksheet_type ws =  m_ws.lock();
    if ( ! ws ) {
        throw invalid_worksheet_exception();
    }
    ws->conditional_formats().update( m_session, m_id, m_cf_format );
}

DEFINE_CLONEABLE( update_conditional_format_command );
