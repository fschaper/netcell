...
struct check_if_na {
    static bool check_for_non_default( const variant& v ) {
        return !v.is_error() || v.error_code() != variant::error_n_a;
    }
};
template<>
bool check_if_na<false>::check_for_non_default( const variant& v )
{
    return !check_if_na<true>::check_for_non_default( v );
}
...
interpreter_variant function_isna( function_parameter& parameters )
{
    ...
    return function_helper::create_array_or_return_single_value<check_if_na>( parameters[0] );
    // Returns true, if the argument refers to the #N/A (value not available) error value.
}
