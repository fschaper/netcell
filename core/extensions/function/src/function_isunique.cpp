...
*
* \returns
* an array of bool the same size of input depicting the unicity
*
* \details
*
*
* \see
* function_isduplicate
*
* \author
* Radu Ialovoi <ialovoi@yalos-solutions.com>
*/
interpreter_variant function_isunique( function_parameter& parameters )
{
    ...
    if ( variant::type_array == parameters[0].type() ) {
            ...
            for ( ; j != e; ++j ) {
                ...
                if ( i->second == j->second ) {
                    ...
                    *( markers.get() + pos2 ) = true;
                    resultArray.set( j->first, variant( false ) );
                }
            }
        ...
        return interpreter_variant( variant( resultArray, static_cast<unsigned int>( parameters[0].array().size() ) ) );
        // Generates an array of logical variants that states the unicity of individual iterm in the input.
    }
    return interpreter_variant( !parameters[0].is_empty() );
