 ...
 * Frieder Hofmann <frieder.hofmann@jedox.com>
 */
class WSS_MODULE get_border_command
    : public base_command<i_command>
{
    typedef std::vector<border_style_flyweight_type> border_style_vec;
public:
    typedef const border_style_vec& result_type;

    get_border_command( session& s, const sheet_range rect );
    ...
    DECLARE_CLONEABLE( get_border_command )

private:
    sheet_range m_rect;
    weak_worksheet_type m_worksheet;
    border_style_vec m_value;
};
