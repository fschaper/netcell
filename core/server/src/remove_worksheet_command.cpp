...
remove_worksheet_command::remove_worksheet_command( session& s, const uuid_type& id_to_remove /* = boost::uuids::nil_uuid */ )
    : base_command<i_command>::base_command( s )
    , m_id_to_remove( id_to_remove )
{}

void remove_worksheet_command::execute()
{
    shared_workbook_type wb =  m_session.selected().workbook().lock();
    if ( ! wb ) {
        throw invalid_workbook_exception();
    }
    wb->remove( m_id_to_remove );
    m_session.clear_cache();
}

DEFINE_CLONEABLE( remove_worksheet_command );
