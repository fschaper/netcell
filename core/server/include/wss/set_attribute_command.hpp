 ...
 * Florian Schaper <florian.schaper@jedox.com>
 */
class WSS_MODULE set_attribute_command
    : public abstract_undoable_command
{
public:
    typedef void result_type;

    set_attribute_command( session& s, const sheet_range& area, const std::string& json );
     ...
     * Florian Schaper <florian.schaper@jedox.com>
     */
    DECLARE_CLONEABLE( set_attribute_command );

private:
    sheet_range m_area;
    std::string m_new_attributes;
#ifdef CORE_DO_UNDO_SUPPORT
    attribute_manager::undo_list_type m_old_attributes;
#endif
};
