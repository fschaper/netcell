...
unmerge_cells_command::unmerge_cells_command( session& s, const sheet_range& range )
    : abstract_undoable_command( s ), m_range( range ), m_ws( s.selected().worksheet() )
{}

void unmerge_cells_command::execute()
{
...
void unmerge_cells_command::un_execute()
{
    ...
    //TODO this is not always the right way to undo.
    ws->range( m_range ).merge( m_session );
#endif
}

DEFINE_CLONEABLE( unmerge_cells_command );
