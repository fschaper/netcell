...
move_worksheet_command::move_worksheet_command( session& s, const std::string& ws_id, const std::string& next_ws_id, const std::string& target_wb_id )
...
void move_worksheet_command::execute()
{
    ...
    if ( target_wb == source_ws->parent() ) {
        target_wb->move( m_session, source_ws->uuid(), m_next_id );
        ws = source_ws;
    } else {
        ws = source_ws->copy( m_session, target_wb, m_next_id );
        source_ws->parent()->remove( source_ws->uuid() );
    }
    m_moved_sheet_id = boost::lexical_cast<std::string>( ws->uuid() );
    m_session.selected().worksheet( ws );
}

move_worksheet_command::result_type move_worksheet_command::value() const
{
    return m_moved_sheet_id;
}

DEFINE_CLONEABLE( move_worksheet_command );
