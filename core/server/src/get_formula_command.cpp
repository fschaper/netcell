...
get_formula_command::get_formula_command( session& s, const sheet_range& p )
    : base_command<i_command>::base_command( s )
    , m_position( p )
{}

void get_formula_command::execute()
{
    ...
    m_value = r.formula( m_session );
}

get_formula_command::result_type get_formula_command::value() const
{
    return m_value;
}

DEFINE_CLONEABLE( get_formula_command );
