...
struct check_if_err {
    static bool check_for_non_default( const variant& v ) {
        return !v.is_error() || variant::error_n_a != v.error_code();
    }
};

template<>
bool check_if_err<false>::check_for_non_default( const variant& v )
{
    return !check_if_err<true>::check_for_non_default( v );
}
...
interpreter_variant function_iserr( function_parameter& parameters )
{
    ...
    return function_helper::create_array_or_return_single_value<check_if_err>( parameters[0] );
    // Returns true, if the argument refers to any error value except #N/A
}
