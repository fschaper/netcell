 ...
 *
 * \version
 * SVN: $Id: Chart.js 5140 2011-07-22 10:44:05Z predragm $
 *
 */

Jedox.wss.wsel.chart = new function() {
	this.type_sizeDefs = {
	...
	// move and if needed resize chart - called by WMV command
	this.reMovize = function(chartId, pos, offsets) {
		var chart = Jedox.wss.wsel.wselRegistry.get(Jedox.wss.app.activeBook, Jedox.wss.app.activeSheet._uid, chartId),
			...
			// get TopLeft and Size of wsel
			tlSize = Jedox.wss.wsel.getTLSize(pos, offsets),
			w = tlSize.size[0], h = tlSize.size[1];

		// set new position of element
		for (var i = chart.wrapper.length - 1, wrpEl; i >= 0; i--)
			(wrpEl = chart.isUserMode ? chart.wrapper[i].getEl() : chart.wrapperEl[i]).setLeftTop(''.concat(tlSize.tl[0], 'px'), ''.concat(tlSize.tl[1], 'px'));

		// Check is width and height in proper range
		var whRatio = Jedox.wss.wsel.chart.min_whRatio[Jedox.wss.wsel.chart.type_sizeDefs[chart.eElem[chart._sheet._aPane._id].elemSubtype]];
		w = (whRatio[0] * Jedox.wss.wsel.chart.min_sizeFactor > w) ? whRatio[0] * Jedox.wss.wsel.chart.min_sizeFactor : ((Jedox.wss.wsel.chart.max_width < w) ? Jedox.wss.wsel.chart.max_width : w);
		h = (whRatio[1] * Jedox.wss.wsel.chart.min_sizeFactor > h) ? whRatio[1] * Jedox.wss.wsel.chart.min_sizeFactor : ((Jedox.wss.wsel.chart.max_height < h) ? Jedox.wss.wsel.chart.max_height : h);

		// Show Mesg that size is not good
...
//			});
		...
		if ((wrpEl.dom.offsetWidth != w) || (wrpEl.dom.offsetHeight != h) || (pos[0] == pos[2]) || (pos[1] == pos[3])) {
			var chartData = Jedox.wss.wsel.getNLoc(tlSize.tl[0], tlSize.tl[1], w, h);

			if (tlSize._error)
				var res = Jedox.backend.wss.wsel('resize_chart', {id: chartId, n_location: chartData.n_location, pos_offsets: chartData.pos_offsets, size: [w, h]});
			...
			for (var i = chart._panes.length - 1; i >= 0; i--) {
				...
				(chart.isUserMode ? chart.wrapper[i].getEl() : chart.wrapperEl[i]).setSize(w, h);
	...
	this.createChart = function (chartId, n_location, pos_offsets, size, chartType, zindex, locked, name) {
		Jedox.wss.wsel.wselRegistry.add(new Jedox.wss.wsel.Chart(chartId, n_location, pos_offsets, size, chartType, zindex, locked, name));
	};
};

Jedox.wss.wsel.Chart = (function () {

	// private static fields
	var type = 'Chart';

	// class constructor
	return function (chartId, n_location, pos_offsets, size, chartType, zindex, locked, name) {
		Jedox.wss.wsel.Chart.parent.constructor.call(this, {
			id: chartId,
			name: name,
			type: type,
			zindex: zindex,
		...
		if (this.conf.locked == undefined) {
			this.conf.locked = Jedox.wss.wsel.Chart.defLockedState;
		...
		// get TopLeft and Size of Element
		var tlSize = Jedox.wss.wsel.getTLSize(Jedox.wss.wsel.getRngFromNLoc(this.conf.n_location), this.conf.pos_offsets);
		...
		// Check is width and height in proper range
		var whRatio = Jedox.wss.wsel.chart.min_whRatio[Jedox.wss.wsel.chart.type_sizeDefs[this.conf.chartType]];
		this.conf.width = (whRatio[0] * Jedox.wss.wsel.chart.min_sizeFactor > this.conf.width) ? whRatio[0] * Jedox.wss.wsel.chart.min_sizeFactor : ((Jedox.wss.wsel.chart.max_width < this.conf.width) ? Jedox.wss.wsel.chart.max_width : this.conf.width);
		this.conf.height = (whRatio[1] * Jedox.wss.wsel.chart.min_sizeFactor > this.conf.height) ? whRatio[1] * Jedox.wss.wsel.chart.min_sizeFactor : ((Jedox.wss.wsel.chart.max_height < this.conf.height) ? Jedox.wss.wsel.chart.max_height : this.conf.height);

		// Show Mesg that size is not good
		if (tlSize._error || (this.conf.width != tlSize.size[0]) || (this.conf.height != tlSize.size[1]))
		{
//			Ext.Msg.show({
//				title: "Incorect Chart Size".localize(),
...
//			});
			...
			var chartData = Jedox.wss.wsel.getNLoc(tlSize.tl[0], tlSize.tl[1], this.conf.width, this.conf.height);
			var res = Jedox.backend.wss.wsel('resize_chart', {id: this.conf.id, n_location: chartData.n_location, pos_offsets: chartData.pos_offsets, size: [this.conf.width, this.conf.height]});
		...
		for (var i = 0, panesLen = this._panes.length, gridDivId, elContId, pane, contId; i < panesLen; i++) {
			...
			if (!this.isUserMode) {
				this.wrapper[i].on({
					'resize': {fn: this.afterResizeChart, scope: this},
				...
				this.wrapperEl[i].on({
					'mousedown': {fn: this.onChartMouseDown, scope: this, options: {paneId: i}},
				...
				this.wrapper[i].getEl().on({
					'mousedown': {fn: this.onMouseDownUM, scope: this, options: {paneId: i}}
				});
		}
	}
}
...
// Chart extends Base
Jedox.util.extend(Jedox.wss.wsel.Chart, Jedox.wss.wsel.Base);

//public fields
Jedox.wss.wsel.Chart.defLockedState = false;
Jedox.wss.wsel.Chart.dims = {maxW: 700, maxH: 700};

//public methods

clsRef = Jedox.wss.wsel.Chart;
...
clsRef.prototype.beforeResize = function(wrapper, e) {
	var resizeChild = wrapper.getResizeChild(),
		...
		whRatio = Jedox.wss.wsel.chart.min_whRatio[Jedox.wss.wsel.chart.type_sizeDefs[resizeChild.elemSubtype]];
...
clsRef.prototype.afterResizeChart = this.afterResize = function (wrapper, newWidth, newHeight, e) {
	...
	var chartData = Jedox.wss.wsel.getNLoc(this.conf.left, this.conf.top, this.conf.width, this.conf.height);
	var res = Jedox.backend.wss.wsel('resize_chart', {id: this.conf.id, n_location: chartData.n_location, pos_offsets: chartData.pos_offsets, size: [this.conf.width, this.conf.height]});
	...
	for (var i = this._panes.length - 1; i >= 0; i--)
		document.getElementById(this._panes[i]._domId.concat('_wsel_cont_', this.conf.id)).src = '/be/wss/gen_element.php?wam='.concat(
			Jedox.wss.app.appModeS,
			'&buid=', this._book.uid,
			'&suid=', this._sheet.getUid(),
			'&id=', this.conf.id,
...
clsRef.prototype.edit = function() {
	Jedox.gen.load(Jedox.wss.app.dynJSRegistry.editChart, ['edit', this.conf.id]);
};

clsRef.prototype.editType = function() {
	Jedox.gen.load(Jedox.wss.app.dynJSRegistry.editChartType, ['edit', this.conf.id]);
};

clsRef.prototype.editSourceData = function() {
	Jedox.gen.load(Jedox.wss.app.dynJSRegistry.editSourceData, ['edit', this.conf.id]);
};

clsRef.prototype.remove = function() {
	if (Jedox.backend.wss.wsel('delete_element', this.conf.id)) {
...
clsRef.prototype.showContextMenu = function (e, paneId, pos) {
	...
	var contextMenu = new Ext.menu.Menu({
		...
		items: this.isUserMode ? [] : [
			{id: 'wChartContext_edit_btn', text: 'Change Chart Type'.localize().concat('...'), iconCls: 'icon_insert_chart', scope: this, handler: this.editType},
			{id: 'wChartContext_selSrcData_btn', text: 'Select Source Data'.localize().concat('...'), iconCls: 'chart_source_data', scope: this, handler: this.editSourceData},
			{id: 'wChartContext_format_btn', text: 'Format Chart Properties'.localize().concat('...'), iconCls: 'icon_edit', scope: this, handler: this.edit},
			'-',
			new Ext.menu.Item(
				{id: 'wChartContext_bToFrnt_btn', text: "Bring to Front".localize(), iconCls: 'ico_bring_to_front', disabled: this.conf.locked, menu: {
						items: [
							{id: 'wChartContextSub_bToFrnt_btn', text: "Bring to Front".localize(), iconCls: 'ico_bring_to_front', scope: this, handler: function() {Jedox.wss.wsel.wselRegistry.bringToFront(this.conf.id);}},
							{id: 'wChartContextSub_bFwd_btn', text: "Bring Forward".localize(), iconCls: 'ico_bring_forward', scope: this, handler: function() {Jedox.wss.wsel.wselRegistry.bringForward(this.conf.id);}}
						]
					}
				}
			),
			new Ext.menu.Item(
				{id: 'wChartContext_sToBck_btn', text: "Send to Back".localize(), iconCls: 'ico_send_to_back', disabled: this.conf.locked, menu: {
						items: [
							{id: 'wChartContextSub_sToBck_btn', text: "Send to Back".localize(), iconCls: 'ico_send_to_back', scope: this, handler: function() {Jedox.wss.wsel.wselRegistry.sendToBack(this.conf.id);}},
							{id: 'wChartContextSub_sBckwd_btn', text: "Send Backward".localize(), iconCls: 'ico_send_backward', scope: this, handler: function() {Jedox.wss.wsel.wselRegistry.sendBackward(this.conf.id);}}
						]
					}
				}
			),
			{id: this.conf.locked ? 'wChartContext_unlock_btn' : 'wChartContext_lock_btn', text: (this.conf.locked ? 'Unlock' : 'Lock').localize(), iconCls: 'icon_lock', scope: this, handler: function(item, e) {
			...
			'-',
			{id: 'wChartContext_del_btn', text: 'Delete Chart'.localize(), iconCls: 'icon_delete', scope: this, handler: this.remove}
		]
	});

	if (Jedox.wss.app.isIE)
		contextMenu.insert(this.isUserMode ? 0 : 8, new Ext.menu.Item({id: 'wChartContext_cpToClp_btn', text: 'Copy to Clipboard'.localize(), iconCls: 'icon_copy', scope: this, handler: function() {this.copyToClipboard(paneId);}}));
...
//		wrpEl = this.wrapperEl[paneId] || this.wrapper[paneId].getEl();
//
//	return wrpEl[type ? 'getWidth' : 'getHeight'](true);
//};
...
clsRef.prototype.setPosition = function (nLoc, posOffs) {
	if (typeof nLoc == 'string' & nLoc.length)
		this.location = Jedox.wss.wsel.getRngFromNLoc(nLoc);
