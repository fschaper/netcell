...
struct check_if_text {
    static bool check_for_non_default( const variant& v ) {
        return !v.is_string() ;
    }
};
template<>
bool check_if_text<false>::check_for_non_default( const variant& v )
{
    return !check_if_text<true>::check_for_non_default( v );
}
/*!
 * \brief
...
interpreter_variant function_istext( function_parameter& parameters )
{
    ...
    return function_helper::create_array_or_return_single_value<check_if_text>( parameters[0] );
    // Returns true, if the argument refers to a text.
}
