...
/*!
* \brief
* hide column if param 1 is true
*
...
interpreter_variant function_hidecolumn( function_parameter& parameters )
{
    ...
    unsigned int column_no = parameters.context().caller()->position().column();
    ...
    if ( hide ) {
        ...
        ws->layout().hide_column( column_no );

    } else {

        ws->layout().unhide_column( column_no );
