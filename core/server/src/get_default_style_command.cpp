...
get_default_style_command::get_default_style_command( session& s )
    : base_command<i_command>::base_command( s )
{}

void get_default_style_command::execute()
{
    shared_application_type app = m_session.selected().application();
    if ( ! app ) {
        throw invalid_application_exception();
    }
    m_default_style = app->default_style().serialize();
}

get_default_style_command::result_type get_default_style_command::value() const
{
    return m_default_style;
}

DEFINE_CLONEABLE( get_default_style_command );
