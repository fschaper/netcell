...
public:
    ...
    operator variant() {
        if ( 0 == m_count  || m_count == 1 ) {
            return variant( variant::error_div_null );
        }

        double sx = sumx2 - sumx * sumx / m_count;

        // If any standard deviation equals zero, INTERCEPT returns the #DIV/0! error value.
        if ( sx <= 0.0 ) {
            return  variant( variant::error_div_null );
        }

        double slope = ( sumxy - sumx * sumy / m_count ) / sx;
        double result = sumy / m_count - slope * sumx / m_count;
