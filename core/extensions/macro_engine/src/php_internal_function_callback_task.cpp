...
php_internal_function_callback_task::php_internal_function_callback_task( const std::string& function_name , const parameter_list_type& parameters )
    : m_function_name( function_name )
    , m_parameters( parameters )
{}

void php_internal_function_callback_task::operator()() const
{
    TSRMLS_FETCH();
    ...
    zend_first_try {
        ...
        ZVAL_STRING( function_name, const_cast<char*>( m_function_name.c_str() ), 1 );
        ...
        LOG_PHP_ERROR( "failed to execute PHP function '" << m_function_name << "()': unable to execute function" );
        m_result = variant::error_value;
    } zend_end_try();
}

const variant& php_internal_function_callback_task::result() const
{
    return m_result;
}
