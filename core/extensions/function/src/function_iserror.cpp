...
template<>
bool check_if_error<false>::check_for_non_default( const variant& v )
{
    return !check_if_error<true>::check_for_non_default( v );
}

/*!
 * \brief
 * Returns true, if the argument refers to any error value.
 *
 ...
 *
 * \returns
 * logical value, if the argument refers to any error value
 *
 * \details
 *
 * \see
 * function_iserr | function_isna
 *
 * \author
 * Frieder Hofmann <frieder.hofmann@jedox.com>
...
interpreter_variant function_iserror( function_parameter& parameters )
{
    ...
    return function_helper::create_array_or_return_single_value<check_if_error>( parameters[0] );
    // Returns true, if the argument refers to any error value.
}
