...
/*!
 * \brief
 * sets a style for an area on the currently selected worksheet
 *
 * \author
 * Florian Schaper <florian.schaper@jedox.com>
 */
class WSS_MODULE set_style_command
    : public abstract_undoable_command
{
public:
    typedef void result_type;

    set_style_command( session& s, const sheet_range& area, const std::string& css );
     ...
     * Florian Schaper <florian.schaper@jedox.com>
     */
    DECLARE_CLONEABLE( set_style_command );

private:
    sheet_range m_area;
    std::string m_new_style;
#ifdef CORE_DO_UNDO_SUPPORT
    style_manager_type::undo_list_type m_old_style;
#endif
};
