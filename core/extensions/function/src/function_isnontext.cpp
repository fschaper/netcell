...
struct check_if_nontext {
    static bool check_for_non_default( const variant& v ) {
        return v.is_string() || v.is_empty();
    }
};

template<>
bool check_if_nontext<false>::check_for_non_default( const variant& v )
{
    return !check_if_nontext<true>::check_for_non_default( v );
}
...
interpreter_variant function_isnontext( function_parameter& parameters )
{
    ...
    return function_helper::create_array_or_return_single_value<check_if_nontext>( parameters[0] );
    // Returns true, if the argument refers to any item that is not text.
    // Returns also true, if the argument refers to a blank cell.
}
